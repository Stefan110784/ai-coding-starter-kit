...
- **API:** `GET/POST /api/abweichungen` (+ `PATCH /api/abweichungen/[id]`, bewusst **kein DELETE** — Aufzeichnungspflicht), `GET/POST /api/abweichungen/gruende` (POST mit Verwaltungs-Recht). Alle Änderungen laufen über `auditFeldDiff` ins Audit-Log.
