...
- **Hartes Gate** in `PATCH /api/auftraege/[id]`: Übergang → `abgeschlossen` bei Nicht-L-Aufträgen verlangt, dass die **jüngste** Endprüfung `ergebnis ∈ {ok, bedingtFrei}` hat (eine spätere „abweichend"-Prüfung widerruft ältere Freigaben) **und** nach der letzten Reaktivierung liegt (Statuswechsel weg von `abgeschlossen`, via AuditEvent ermittelt) — sonst `409 { error: "pruefungFehlt" }`. Bewusst **ohne force-Umgehung** (anders als das Material-Gate). Greift VOR allen Buchungs-Hooks. L-Aufträge (Lagerfertigung) sind ausgenommen.
- **Nachweis-Härtung (Review):** `auftragNummer` denormalisiert (Protokoll bleibt nach Auftrags-Löschung lesbar); `prueferId` nullable mit SetNull (Benutzer bleiben löschbar, Nachweis bleibt — konsistent mit allen anderen Benutzerstempeln).
