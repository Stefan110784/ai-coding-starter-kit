...
import { auftraegeInWoche, kpiFuerZeitraum, kundenLiefertreueInWoche } from "@/lib/auswertung";
...
export async function GET(req: NextRequest) {
  ...
  const [auftraege, kunden] = await Promise.all([
    auftraegeInWoche(jahr, woche),
    kundenLiefertreueInWoche(jahr, woche),
  ]);
  return ok({
    year: jahr,
    week: woche,
    ...kpiFuerZeitraum(auftraege),
    // Ende-zu-Ende-Liefertreue gegen den Kundenwunschtermin (KF3-37)
    kundenLiefertreueRate: kunden.prozent,
    kundenLiefertreueBasis: kunden.basis,
  });
}
