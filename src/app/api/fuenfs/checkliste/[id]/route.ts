...
const patchSchema = z
  .object({
    ...
    kategorie: z.enum(["seiri", "seiton", "seiso", "seiketsu", "shitsuke"]).optional(),
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    const punkt = await prisma.$transaction(async (tx) => {
      const alt = await tx.fuenfSChecklistenPunkt.findUnique({ where: { id } });
      if (!alt) throw new NichtGefunden();
      const neu = await tx.fuenfSChecklistenPunkt.update({ where: { id }, data: parsed.data });
      await auditFeldDiff(tx, "fuenfsChecklistenPunkt", id, auth.benutzer.id, alt, parsed.data, [
        "text",
        "kategorie",
        "aktiv",
      ]);
    ...
    if (e instanceof NichtGefunden) return err("Punkt nicht gefunden", 404);
